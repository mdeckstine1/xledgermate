...
def test_stale_pending_sell_cancelled_when_ask_drifts(bracket_state):
    """Zombie inventory asks must free sell slots for autonomous trims."""
    ...
    async def _run() -> None:
        ...
        # Far above mid (+8%) — classic zombie that bricked max_pending_sells.
        ledger._offers[800] = {
            "sequence": 800,
            "side": "ask",
            "price": 1.091,
        ...
        mgr.register_strength_sell(
            sequence=800,
            size_xrp=40.0,
            price_rlusd_per_xrp=1.091,
            purpose="strength",
        )

        await mgr.sync_brackets()

        assert 800 in ledger.cancelled
        assert mgr._strength_sells.get(800) is None

    asyncio.run(_run())


def test_stale_pending_sell_kept_near_target(bracket_state):
    from alpha.decision.structure import MarketStructureSnapshot
    from alpha.orders.stale_pending import target_sell_limit_price
    ...
    async def _run() -> None:
        ...
        target = target_sell_limit_price(mid, 0.08)
        ledger._offers[801] = {
            "sequence": 801,
            "side": "ask",
            "price": target,
        ...
        assert ledger.cancelled == []
        assert mgr._strength_sells.get(801) is not None
