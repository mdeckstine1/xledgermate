...
def test_maximize_preset_route(client):
    r = client.post("/operator/maximize")
    assert r.status_code == 200
    data = r.json()
    assert data["ok"] is True
    assert data["config_effective"]["inventory_target_xrp_ratio"] == 0.85
    assert data["config_effective"]["alpha_brackets_enabled"] is False
