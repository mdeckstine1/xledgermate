...
**Status (as of 2026-06-20, VPS soak):** Production **`ws-engine`** + **HUD `:8765`** on Ashigaru branch · **v2.1.22** engine · **G6 v1.1.0** HUD. Pure A-S + WS BookFeed + G2/G4/G7 + competitor/peer intel. HUD tabs: **Live**, **Inventory**, **Intelligence**, **Peer Cal**, **Metrics**, **Book**, **Reports**, **Credentials**, **Config**. **Peer Cal** (passive watch): E3 shadow lane vs live pilot band — calibration + Grok/nicknames; log `logs/peer_lane_calibration.jsonl`; **0 peers in band** → P4–P6 shelved. RLUSD-stable **wealth sidebar** (session Δ, skim/spot/rebal). **Book** tab: depth chart + L1–L3 ladder (L2/L3 planned until ledger sync). **Metrics**: G3 §7 grades + **G6 v1.1 activation** (session-scoped since `session_boot_utc`). **G8** spot trend posture spec in [`STRATEGY_MANUAL.md`](STRATEGY_MANUAL.md) — deferred. Grok analyze live when configured. Sacred HTTP-poll long-run untouched. AI / Grok is **strictly advisory** — never overrides A-S reservation.
...
- **Metrics tab**: Phase E §7 grades (spread capture primary; inventory steering). **G6 activation** card — tier (`warming_up` → `pilot` → `thin_edge` / `pilot_watch` → `active` → `scale_ready`; **`hold`** only on bad economics with n≥15; **`halted`** on kill). Yellow `thin_edge` = join-aligned thin book, gate PASS — not scale_ready. Red hold = bad economics advisory only.
...
**G6 activation (Metrics + soak strip):** Grades **session fills since `session_boot_utc`** when present (0–7 fills → **`warming_up`**; does not inherit cumulative CSV **hold** after restart). **v1.1.0 shipped (HUD-only):** `thin_edge` (5–8 bps, ≥70% pos) → gate **PASS** (yellow); **`hold`** only when n≥15 and bad economics (bps&lt;0, pos&lt;50%, or bps&lt;3 and pos&lt;70%) — advisory, does not halt quoting. Spec + acquisition pivot: [`PURE_AS_CRITICAL_PATH.md`](PURE_AS_CRITICAL_PATH.md). Code tree: [`G6_ACTIVATION_REFERENCE.md`](G6_ACTIVATION_REFERENCE.md). CLI: `python -m experimental.ws_feed.live_activation_grading --gate`.
