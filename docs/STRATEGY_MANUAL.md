...
- Config tab (in the real-time HUD) drives Grok/xAI tokens (provider=grok, real xai-... key, model=grok-beta, enabled). "Apply" pushes live; the dedicated `/analyze_competitor` endpoint (and the Intelligence tab "Analyze with AI" button) does real calls on specific competitor ledger addresses for trending/strategy summaries.
