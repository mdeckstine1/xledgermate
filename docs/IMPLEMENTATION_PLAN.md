...
*Updated: 2026-06-10 · v1.4.4 lineage · Transitioning from hard gates to **WS + pure A-S** (experimental on `grok-ws-feed`; VPS Gate 2 sacred until swap sign-off).*

**Active work checklist:** [`PURE_AS_CRITICAL_PATH.md`](PURE_AS_CRITICAL_PATH.md) — single source for Phase A–E tasks.  
**Run commands / file pack:** [`groks input/CURSOR_HANDOFF_ROADMAP.md`](../groks%20input/CURSOR_HANDOFF_ROADMAP.md).  
Sacred gated long-run data remains the validation corpus only.
...
## Tier 3 — WS + pure A-S (committed production path)

**We are moving away from hard `market_edge_met` gates toward pure Avellaneda-Stoikov + WS book feed.** All implementation tasks live in [`PURE_AS_CRITICAL_PATH.md`](PURE_AS_CRITICAL_PATH.md). Work stays in `experimental/ws_feed/` until post–Gate 2 wholesale swap. The sacred long-run (HTTP poll + hard gate on VPS) is **data generator only** — not modified during Gate 2.
...
- **Current code position & gaps for achieving the target**:
  ...
  - **Pressure model landed (2026-06-09)**: `experimental/competitor_pressure.py` — `CompetitorPressure` (value 0.0 defensive/wide = skim harder; side-aware ask/bid for XRP-heavy rebalance via `effective_for_inventory`). `apply_competitor_pressure` produces `PressureAdjustedInputs` (monotonic lower vol + higher size_mult + gamma_scale + observed-spread book anchor when low pressure). Integrated in `engine_adapter_example.py` as the PureQuotePath: accepts `competitor_intel`, applies before pure A-S call (vol/book/gamma/size only — reservation and inside-book `as_met` untouched). Outputs pressure_* fields + rationale in note. Tests cover low/high, observed spread, XRP-heavy ask pressure.
  - Gaps (main engine still carries legacy; need to close for full pure/predator scaling): market_edge_met, "thin book → near-touch backoff", "edge guard size reduction", "market edge thin → widen", "hostile + weak edge → pause", "defensive only" still active in dynamic_quoting_policy.py, quote_decision.py, market_microstructure.py, trading_engine, etc. (even if experimental path aims to bypass). AI advisory (now hooked in adapter after pressure) mostly stub/demo; full `AIAdvisorySignal` + real analyzer wiring + economics A/B validation next. No automatic dynamic order_size ramp tied to current capital + pressure yet in main paths. WS age used for monitoring but not yet aggression modulator.
  - To hit P&L / be the predator (skim harder, higher presence, beat competitors): ensure the WS pure path *actually* bypasses legacy outer gates for this deployment (force market_edge_met=True + skip size/edge reductions when as_mode=="pure"). Wire competitor_pressure deeply (low pressure → lower effective gamma / use observed spread / boost L1 size / more presence exactly where competitors are weak) — done in adapter. Add dynamic sizing (L1 = min(configured, 0.06–0.08 * current XRP bal); boost asks during XRP-heavy rebalance). Use live WS (fresh age + depth) + pressure + AI advisory to decide "predator mode" (quote more / size up). Leverage AI/Grok (already in Config + /analyze_competitor) for competitor address trending to inform "go harder on this maker/level" (advisory). `AIAdvisorySignal` now in base + hooked in adapter (further vol/size mult after pressure, attached to output). These turn the bot from "safe high-presence MM" into the aggressive skimmer that capitalizes on defensive books/competitors while the A-S math protects. Run full sacred_economics A/B on pressure+AI before live claims.
...
*Further 11k / predator extrapolations and open tasks: see [`PURE_AS_CRITICAL_PATH.md`](PURE_AS_CRITICAL_PATH.md) Phase A–E (P&L targets are hypothesis until economics A/B + live fills validate).*
...
**Actionable tasks:** [`PURE_AS_CRITICAL_PATH.md`](PURE_AS_CRITICAL_PATH.md) Phase A–E. Vision detail: [`experimental/ai_analysis/THE_AI_DISCUSSION.md`](../experimental/ai_analysis/THE_AI_DISCUSSION.md).
...
| 2026-06-10 | **Pure A-S critical path** | Consolidated docs; [`PURE_AS_CRITICAL_PATH.md`](PURE_AS_CRITICAL_PATH.md) is the single WS + pure A-S task checklist; moving from hard gates to pure A-S on `grok-ws-feed`. |
