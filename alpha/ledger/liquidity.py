...
def depth_within_mid_band(
    ...
    *,
    ...
    """Sum XRP on one side within band_pct of mid (HUD market conditions)."""
    if not levels or mid <= 0 or band_pct <= 0:
        return 0.0
    if side == "bid":
        floor = mid * (1.0 - band_pct / 100.0)
        return sum(max(0.0, level.size_xrp) for level in levels if level.price >= floor)
    if side == "ask":
        ceiling = mid * (1.0 + band_pct / 100.0)
        return sum(max(0.0, level.size_xrp) for level in levels if level.price <= ceiling)
    raise ValueError(f"Unsupported side: {side}")
