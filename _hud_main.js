        ...
        function spotMarqueeText(s) {
            ...
            if (spotPct != null && !Number.isNaN(spotPct)) {
                ...
                line += ` · Δmid ${sign}${spotPct.toFixed(2)}%`;
            }
        ...
        function renderLiveInner(s) {
            ...
            if (wealthDeltaEl) {
                if (wDelta != null && !Number.isNaN(wDelta)) {
                    wealthDeltaEl.textContent = `Session Δ ${formatSignedRlusd(wDelta, 2)}`;
                    wealthDeltaEl.className = 'sidebar-value ' + wealthDeltaClass(wDelta);
                } else {
                    wealthDeltaEl.textContent = 'Session Δ: —';
            ...
            setText(
                ...
                spotR != null && !Number.isNaN(spotR)
                    ? `Spot (XRP×Δmid): ${formatSignedRlusd(spotR, 2)}`
                    : 'Spot (XRP×Δmid): —'
            );
            ...
            if (spotPct != null && !Number.isNaN(spotPct)) {
                ...
                base += `   Spot Δmid <span style="${spotStyle}">${sign}${spotPct.toFixed(2)}%</span>`;
            }
            if (spotR != null && !Number.isNaN(spotR)) {
                const sign = spotR >= 0 ? '+' : '';
